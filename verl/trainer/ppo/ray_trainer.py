...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # Aman modified
            input_texts = [x.strip().removeprefix("user").removesuffix("assistant").strip() for x in input_texts]
            ...
            output_texts = [x.replace("<|im_end|>", "").replace("<|eot_id|>", "") for x in output_texts]
            output_texts = [x.replace("<|endoftext|>", "") for x in output_texts]
            sample_outputs.extend(output_texts)

            test_batch = test_batch.union(test_output_gen_batch)

            # evaluate using reward_function
            result = self.val_reward_fn(test_batch, return_dict=True, is_val=True) # Aman modified
    ...
    def init_workers(self):
        ...
        if self.hybrid_engine:
            ...
            actor_rollout_cls = RayClassWithInitArgs(
                cls=self.role_worker_mapping[Role.ActorRollout], # Role.ActorRollout=2
    ...
    def fit(self):
        ...
        if self.config.trainer.get("use_div_branching", True):
            from verl.div_branching.branching_strategy import BranchingRolloutStrategy
            # 初始化分支采样策略
            branching_strategy = BranchingRolloutStrategy(
                config=self.config,
                tokenizer=self.tokenizer,
                k_branches=self.config.trainer.get("k_branches", 32),
                diversity_metric=self.config.trainer.get("diversity_metric", "ngram"),
            )
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        if not self.async_rollout_mode:
                            if self.config.trainer.get("use_div_branching", True):
                                # 使用分支采样策略进行生成
                                from verl.div_branching.branching_strategy import sync_rollout_with_branching
                                gen_batch_output = sync_rollout_with_branching(
                    ...
                    if rollout_data_dir:
                        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
                            ...
                            inputs = [x.strip().removeprefix("user").removesuffix("assistant").strip() for x in inputs]
                            # outputs = self.tokenizer.batch_decode(batch.batch["responses"], skip_special_tokens=True)
                            outputs = [self.tokenizer.decode(x, skip_special_tokens=False).replace("<|im_end|>", "").replace("<|eot_id|>", "").replace("<|endoftext|>", "") for x in batch.batch["responses"]]
